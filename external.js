...
const config = {
    ...
    mnemonic: 'tomorrow magic deal control flight quote left saddle type domain rifle aware',
};
...
const contractAddr = "juno1lqqv9qt5ghlpzsy0wsk02zh0qdansm8fkh9rjz97ke4zvh78254qx80jj6"
const rewardtokenAddr = "juno1y9rf7ql6ffwkv02hsgd4yruz23pn4w97p75e2slsnkm0mnamhzysvqnxaq"
...
function log(logstr) {
    ...
    fs.appendFileSync(`/achilles/external-log/external-${moment(new Date()).format('YYYY-MM-DD')}.log`, logstr + "\n")
    console.log(logstr)
}
...
async function main() {
    ...
    let current_amount = await client.queryContractSmart(
        rewardtokenAddr,
    ...
    if (history.length > 0) {
        ...
        if (Math.floor(last / step) < Math.floor((new Date().getTime() / 1000 / step)))
            flag = true
    } else {
        flag = true
        first = true
    ...
    while (history.length > 60)
        history.shift()
    try {
        fs.writeFileSync(`history.js`, "export const history = " + JSON.stringify(history, null, 4))
    } catch (err) {
        console.error(err)
    }
    if (!flag)
    ...
    xmlHttp.open( "GET", 'https://lcd-juno.cosmostation.io/wasm/contract/' + contractAddr + '/state', false ); // false for synchronous request
    xmlHttp.send( null );
    ...
    for (let i = 0; i < dao_staker_list.length; i ++) {
        let reward_amount = Math.floor(daily_reward * parseInt(dao_staker_list[i].amount) / dao_staker_amount)
        if (reward_amount == 0)
            continue
        ...
        if (msglist.length >= 20 || i == dao_staker_list.length - 1) {
            ...
            log("sending " + total_reward_amount + " BLOCK to " + msglist.length + " stakers start")
            for (let k = 0; k < 20; k ++)
            {
                try {
                    let result = await client?.signAndBroadcast(address, msglist, defaultFee)
                    log("sent " + total_reward_amount + " BLOCK to " + msglist.length + " stakers")
                    break;
                } catch (error) {
                    log(error.toString())
                    await sleep(20000)
